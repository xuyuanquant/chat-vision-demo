...
Python 3.10+，`requests` HTTP 客户端，`chat-vision-sdk==0.1.1` SDK 客户端，标准库 `http.server` 本地 Viewer，SSE 推送。可选依赖：`mss` 用于 Windows 屏幕截图，`qrcode[pil]` 用于二维码，`pytest` 用于测试。
...
SDK 驱动已接入 `chat-vision-sdk==0.1.1`：复用 SDK 的 `ChatVision`、sessions、frame push/status、messages cursor、close、delete 能力，并将 SDK dataclass 返回值转换成本地 Viewer 现有的 dict 状态结构。`CHAT_VISION_DRIVER=sdk` 或 `--driver sdk` 可启用；如果 SDK 未安装，会给出明确错误，不会静默 fallback 到 HTTP。
...
- SDK 驱动已接入 `chat-vision-sdk==0.1.1`；本仓库不包含 SDK 源码、解析内核或生产服务实现。
